...
export function DesktopAppDashboard() {
  ...
  return (
    <div className="flex h-screen w-screen bg-[#060913] text-[#F8FAFC] font-sans select-none overflow-hidden">
      {/* ========================================================================= */}
      {/* 1. LEFT NAVIGATION SIDEBAR (Glideo Obsidian & Sky-Blue Design System)   */}
      {/* ========================================================================= */}
      <aside className="w-60 flex-shrink-0 bg-[#080D1A] border-r border-white/[0.08] flex flex-col justify-between p-4 z-20">
        <div className="space-y-6">
          {/* Glideo Twin-Star Workspace Header */}
          <div className="flex items-center gap-3 p-2.5 rounded-xl bg-white/[0.03] border border-white/[0.08] hover:border-rose-400/30 transition-all cursor-pointer group">
            {/* Generated High-Res Glideo App Logo */}
            <div className="relative w-8 h-8 rounded-xl overflow-hidden shadow-[0_0_15px_rgba(251,113,133,0.35)] border border-rose-400/30 group-hover:scale-105 transition-transform duration-300 flex-shrink-0">
              <Image
                src="/logo.png"
                alt="Glideo"
                width={32}
            ...
            <div className="flex flex-col min-w-0">
              ...
              <span className="text-[10px] text-rose-300/80 font-mono">Desktop Edition</span>
            </div>
          </div>
          ...
          <nav className="space-y-1.5">
            ...
            >
              <Home className={`w-4 h-4 ${activeTab === "home" ? "text-rose-400" : "text-slate-400"}`} />
              <span>Studio</span>
            </button>
            ...
            >
              <div className="flex items-center gap-3">
                <Film className={`w-4 h-4 ${activeTab === "projects" ? "text-rose-400" : "text-slate-400"}`} />
                <span>Projects</span>
              </div>
              {savedProjects.length > 0 && (
                <span className="text-[10px] font-mono px-2 py-0.5 rounded-md bg-rose-500/15 text-rose-300 border border-rose-400/20">
      ...
      <main className="flex-1 flex flex-col min-w-0 bg-[#060913] overflow-y-auto">
        ...
        <header className="h-16 flex-shrink-0 px-6 sm:px-8 border-b border-white/[0.08] flex items-center justify-between gap-6 bg-[#080D1A]/80 backdrop-blur-xl sticky top-0 z-30">
          ...
          <div className="relative flex-1 max-w-md">
            ...
            <input
              ...
              className="w-full bg-[#0B1020] border border-white/10 focus:border-rose-400/60 rounded-full pl-10 pr-4 py-2 text-xs text-white placeholder:text-slate-500 outline-none transition-colors"
            />
          </div>
          ...
          <div className="flex items-center gap-4 flex-shrink-0">
            ...
            >
              <Avatar className="h-8 w-8 ring-1 ring-rose-400/40 group-hover:ring-rose-400 transition-all bg-[#0E1526]">
                <AvatarImage src="/avatar.jpg" alt="Eric Va" />
                <AvatarFallback className="bg-[#0E1526] text-xs font-bold text-white group-hover:text-rose-300">EV</AvatarFallback>
              </Avatar>
            </div>
          </div>
        </header>
        ...
        <div className="p-6 sm:p-8 space-y-8 flex-1 max-w-7xl mx-auto w-full">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <button
              onClick={() => handleCreateNew("16:9")}
              className="p-4 rounded-2xl bg-[#080D1A]/90 hover:bg-[#0E152B] border border-white/[0.08] hover:border-rose-400/50 transition-all text-left group flex items-start gap-3.5 cursor-pointer shadow-sm"
            >
              <div className="w-10 h-10 rounded-xl bg-rose-500/15 border border-rose-400/30 flex items-center justify-center text-rose-400 flex-shrink-0 group-hover:scale-105 group-hover:bg-rose-500 group-hover:text-slate-950 transition-all">
          ...
          <div className="relative rounded-3xl bg-gradient-to-r from-[#080D1A] via-[#0D152B] to-[#080D1A] border border-white/[0.08] p-6 sm:p-10 overflow-hidden shadow-2xl flex flex-col md:flex-row items-center justify-between gap-8">
            ...
            <div className="absolute right-0 top-0 w-96 h-96 bg-rose-500/[0.08] blur-[100px] pointer-events-none" />
            ...
            >
              ...
              <div className="w-36 h-36 rounded-full border border-rose-400/20 flex items-center justify-center relative">
                <div className="w-28 h-28 rounded-full bg-[#0E152B]/80 border border-rose-400/30 flex items-center justify-center backdrop-blur-md">
                  {/* Play Button Icon */}
            ...
            <div className="flex-1 max-w-xl text-left space-y-4">
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-rose-500/10 border border-rose-400/20 text-rose-300 text-xs font-mono">
          ...
          <div className="space-y-4 pt-2">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-mono text-rose-300 bg-rose-500/15 border border-rose-400/20 px-2 py-0.5 rounded-full">
                  {savedProjects.length}
                </span>
              </div>
            </div>
            ...
            {filteredProjects.length === 0 ? (
              ...
              <div className="p-12 rounded-2xl border border-white/[0.08] bg-[#080D1A]/50 backdrop-blur-md text-center flex flex-col items-center justify-center space-y-4">
                <div className="w-14 h-14 rounded-2xl bg-[#0E152B] border border-white/10 flex items-center justify-center text-rose-400 shadow-[0_0_25px_rgba(251,113,133,0.15)]">
                  <Film className="w-7 h-7 text-rose-400 stroke-[1.5]" />
                </div>
              ...
              /* Populated Projects Grid in Glideo Colors */
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredProjects.map((project) => (
                  <div
                    key={project.id}
                    onClick={() => router.push(`/editor?id=${project.id}`)}
                    className="group relative rounded-2xl bg-[#080D1A] border border-white/[0.08] hover:border-rose-400/50 hover:bg-[#0D1429] transition-all p-4 cursor-pointer flex items-center justify-between gap-4 shadow-sm"
                  >
                    <div className="flex items-center gap-3.5 min-w-0">
                      <div className="w-12 h-12 rounded-xl bg-[#0E152B] border border-white/10 flex items-center justify-center text-rose-400 flex-shrink-0 group-hover:scale-105 group-hover:bg-rose-500 group-hover:text-slate-950 transition-all">
                      ...
                      <div className="min-w-0">
                        ...
                        <div className="flex items-center gap-2 text-[10px] text-slate-400 font-mono mt-1">
                          <span className="text-rose-400/80">{project.aspectRatio}</span>
                          <span>·</span>
