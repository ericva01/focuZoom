...
export function HowItWorks() {
  ...
  return (
    <section id="how-it-works" className="py-24 lg:py-32 relative bg-[#060913] text-white border-t border-white/[0.08] overflow-hidden select-none">
      ...
      <div className="absolute top-1/3 left-1/4 w-[600px] h-[400px] bg-rose-500/[0.04] rounded-full blur-[160px] pointer-events-none" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 relative">
          {steps.map((item) => {
            ...
            return (
              <div
                key={item.step}
                className="relative rounded-2xl sm:rounded-3xl p-7 bg-[#080D1A]/70 border border-white/10 hover:border-rose-400/40 backdrop-blur-xl transition-all duration-300 hover:shadow-[0_0_35px_rgba(251,113,133,0.12)] flex flex-col justify-between group overflow-hidden"
              >
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <span className="font-mono text-3xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-rose-400 to-orange-300">
                      {item.step}
                    </span>
                    <div className="w-10 h-10 rounded-xl bg-white/[0.04] border border-white/10 flex items-center justify-center text-rose-400 group-hover:scale-105 group-hover:border-rose-400/40 group-hover:bg-rose-500/20 transition-all duration-300">
                      <Icon className="w-4 h-4" />
                    </div>
                  </div>

                  <h3 className="text-lg font-semibold text-white group-hover:text-rose-200 transition-colors">
                    {item.title}
                  </h3>
