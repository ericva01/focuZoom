...
const features = [
  { icon: Maximize2, label: "KEEP THE FOCUS", title: "Every detail, in the spotlight.", description: "Guide attention with smooth camera zooms that bring the important moments closer.", color: "text-[#FF8A4C] bg-rose-300/10" },
    ...
    <section id="features" className="relative border-t border-white/[0.07] bg-[#030509] py-16 text-white sm:py-24">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="mb-10 flex flex-col justify-between gap-5 md:flex-row md:items-end">
          <div><p className="mb-4 text-xs font-medium uppercase tracking-[0.2em] text-[#FF8A4C]">Small touches. Studio quality.</p><h2 className="max-w-xl text-3xl font-semibold leading-tight tracking-tight sm:text-5xl">Everything your recording<br className="hidden sm:block" /> needs to stand out.</h2></div>
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {features.map(({ icon: Icon, ...feature }) => (
            <article key={feature.title} className="group flex flex-col rounded-2xl border border-white/10 bg-[#0a0e17] p-6 transition-colors hover:border-white/20 hover:bg-[#101520]">
              ...
              <Link href="/editor" aria-label={`Try ${feature.title}`} className="mt-auto flex items-center justify-between border-t border-white/10 pt-4 text-xs font-medium text-slate-300 transition-colors hover:text-[#FF8A4C]">Try it in the studio <ArrowUpRight size={16} /></Link>
            </article>
          ))}
        </div>
      </div>
