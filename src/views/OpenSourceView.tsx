...
import { ArrowRight, Code2, Heart, Terminal, Lock, CheckCircle2, Cpu } from "lucide-react";

export function OpenSourceView() {
  const { t, localePath, isKhmer } = useLanguage();

  const tenetIcons = [Heart, Lock, Code2, Cpu];
          ...
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div className="max-w-3xl">
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-[#FFF1E8] border border-[#FF6B2C]/20 text-xs font-semibold text-[#FF6B2C] mb-6">
                ...
                <span>{t.openSourcePage.badge}</span>
              </div>
              <h1 className="text-4xl sm:text-5xl lg:text-6xl font-extrabold tracking-tight text-[#111318] leading-[1.15]">
                {t.openSourcePage.titleMain} <span className="text-[#FF6B2C]">{t.openSourcePage.titleHighlight}</span>.
              </h1>
              ...
              <div className="mt-8 flex flex-wrap items-center gap-4">
                ...
                >
                  ...
                  <span>{t.openSourcePage.githubBtn}</span>
                ...
                >
                  <span>{t.openSourcePage.launchStudio}</span>
                  <ArrowRight size={16} />
                </Link>
              </div>
            </div>
        ...
        <section className="py-20 border-b border-[#E5E7EB] bg-[#F8F9FB]">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              {t.openSourcePage.tenets.map((tenet, idx) => {
                const Icon = tenetIcons[idx % tenetIcons.length];
                return (
                  <div key={idx} className="rounded-2xl border border-[#E5E7EB] bg-white p-6 shadow-xs">
                    <div className="w-10 h-10 rounded-xl bg-[#FFF1E8] text-[#FF6B2C] flex items-center justify-center mb-4">
                      <Icon size={20} />
                    </div>
                    <h3 className="text-base font-bold text-[#111318] mb-2">{tenet.title}</h3>
                    <p className="text-xs text-[#667085] leading-relaxed">{tenet.desc}</p>
                  </div>
                );
              })}
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
              <div className="lg:col-span-6 space-y-6">
                <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-[#F8F9FB] border border-[#E5E7EB] text-xs font-mono text-[#667085]">
                  <Terminal size={14} />
                  <span>{isKhmer ? "ការចាប់ផ្តើមរហ័សសម្រាប់ Developer" : "Developer Quickstart"}</span>
                </div>
                <h2 className="text-3xl font-bold tracking-tight text-[#111318]">
                  {isKhmer ? "Clone និងដំណើរការលើម៉ាស៊ីនក្នុងរយៈពេលប៉ុន្មានវិនាទី" : "Clone and run locally in seconds."}
                </h2>
                <p className="text-sm text-[#667085] leading-relaxed">
                  {isKhmer
                    ? "FucuFlow ត្រូវបានបង្កើតឡើងដោយបច្ចេកវិទ្យា Web ស្តង់ដារ។ មិនត្រូវការ Docker ស្មុគស្មាញ ឬការកំណត់ Database ឡើយ។ គ្រាន់តែ Clone ដំឡើង ហើយដំណើរការជាការស្រេច។"
                    : "FucuFlow is built with standard web technologies. No complex docker containers or database setups required. Just clone, install, and run."}
                </p>
                <div className="space-y-3 text-xs text-[#111318]">
                  <div className="flex items-center gap-2 font-medium">
                    <CheckCircle2 size={16} className="text-[#FF6B2C]" />
                    <span>{isKhmer ? "ដំណើរការបានពេញលេញដោយគ្មានអ៊ីនធឺណិត" : "Works fully offline without an internet connection"}</span>
                  </div>
                  <div className="flex items-center gap-2 font-medium">
                    <CheckCircle2 size={16} className="text-[#FF6B2C]" />
                    <span>{isKhmer ? "គាំទ្រកម្មវិធី Native Desktop ជាមួយគ្រាប់ចុចកាត់សកល" : "Desktop native support for global shortcuts"}</span>
                  </div>
                  <div className="flex items-center gap-2 font-medium">
                    <CheckCircle2 size={16} className="text-[#FF6B2C]" />
                    <span>{isKhmer ? "សហគមន៍សកម្ម ទទួលស្វាគមន៍ Pull Requests" : "Active community roadmap and pull request review"}</span>
                  </div>
                </div>
              </div>
